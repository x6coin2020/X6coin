...
{
    // Convert x6coin:// to x6coin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("x6coin://"))
    {
        uri.replace(0, 12, "x6coin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "x6coin.lnk";
}
...
{
    return GetAutostartDir() / "x6coin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("x6coin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  x6coin-qt [" + tr("command-line options") + "]                     " + "\n";
